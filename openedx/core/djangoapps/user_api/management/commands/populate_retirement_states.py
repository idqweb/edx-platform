...
class Command(BaseCommand):
    ...
    def _validate_new_states(self, new_states):
        ...
        if not set(REQUIRED_STATES).issubset(set(new_states)):
            print(new_states)
            raise CommandError('settings.RETIREMENT_STATES does not contain all required states ({})'.format(REQ_STR))

        # Confirm that the start and end states are in the right places
        if new_states.index(START_STATE) is not 0:
    ...
    def handle(self, *args, **options):
        ...
        to_create, remaining, to_delete = self._delete_old_states_and_create_new(new_states)

        # Report
        print("All states removed and new states added. Differences:")
        print("   Added: {}".format(to_create))
        print("   Removed: {}".format(to_delete))
